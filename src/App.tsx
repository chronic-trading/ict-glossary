...
const NODES = [
  ...
  { id:'EQH',    x:82,  y:42,  c:'#34d399' }, { id:'EQL',    x:90,  y:30,  c:'#34d399' },
  { id:'MSS',    x:8,   y:58,  c:'#34d399' }, { id:'NWOG',   x:25,  y:65,  c:'#fb923c' },
  { id:'SMT',    x:48,  y:62,  c:'#14b8a6' }, { id:'IRL',    x:65,  y:55,  c:'#60a5fa' },
  { id:'ERL',    x:75,  y:65,  c:'#60a5fa' }, { id:'CE',     x:88,  y:58,  c:'#f59e0b' },
  { id:'CBDR',   x:15,  y:78,  c:'#fb923c' }, { id:'LRLR',   x:32,  y:80,  c:'#60a5fa' },
  { id:'BISI',   x:50,  y:78,  c:'#f59e0b' }, { id:'SIBI',   x:65,  y:82,  c:'#f59e0b' },
  { id:'BPR',    x:80,  y:75,  c:'#f59e0b' }, { id:'Kill',   x:22,  y:92,  c:'#fb923c' },
  { id:'Macro',  x:42,  y:90,  c:'#fb923c' }, { id:'Fractal',x:58,  y:88,  c:'#34d399' },
  { id:'iFVG',   x:72,  y:92,  c:'#f59e0b' }, { id:'MSB',    x:5,   y:40,  c:'#34d399' },
]
const EDGES = [
  [0,1],[0,7],[1,6],[2,3],[2,9],[3,11],[4,5],[4,13],[5,17],[6,7],[7,8],
  [8,14],[9,10],[10,11],[12,13],[14,15],[15,16],[16,17],[18,19],[19,20],
  [20,21],[21,22],[23,24],[24,25],[25,26],[0,9],[2,14],[4,8],[6,12],
]
  ...
  return (
    <div style={{
      ...
      <svg
        ...
        style={{ width: '100%', height: '100%', opacity: 0.055 }}
        aria-hidden
      >
        <defs>
          <filter id="glow">
        ...
        {EDGES.map(([a, b], i) => {
          ...
          return (
            <line key={i}
              ...
              stroke="rgba(245,158,11,0.5)" strokeWidth="0.12"
        ...
        {NODES.map((n, i) => (
          <g key={n.id}
            style={{ animation: `float-node-${i % 4} ${14 + (i % 6) * 2}s ease-in-out infinite` }}
            filter="url(#glow)"
          >
            <circle cx={n.x} cy={n.y} r="1.1" fill={n.c} opacity="0.7"/>
            <text x={n.x + 1.6} y={n.y + 0.4}
              fill={n.c} fontSize="1.5" fontFamily="system-ui" fontWeight="900"
              opacity="0.65">
              {n.id}
            </text>
          </g>
      ...
      <div style={{
        ...
        filter: 'blur(80px)', top: '-150px', left: '20%',
        animation: 'orb-drift-1 20s ease-in-out infinite',
      }}/>
      <div style={{
        position: 'absolute', width: 450, height: 450, borderRadius: '50%',
        background: 'radial-gradient(circle, rgba(96,165,250,0.09), transparent 65%)',
        filter: 'blur(70px)', top: '100px', right: '8%',
      ...
      <div style={{
        ...
        background: 'linear-gradient(270deg, rgba(96,165,250,0.03), transparent)',
        pointerEvents: 'none',
      }}/>
    </div>
  )
...
function Ticker() {
  ...
  const full  = [...items, ...items] // double for seamless loop
  return (
    <div style={{
      ...
      <div style={{
        ...
        animation: `ticker-scroll ${items.length * 0.9}s linear infinite`,
        willChange: 'transform',
      }}>
        {full.map((label, i) => (
          <span key={i} style={{
            fontSize: '9px', fontWeight: 900, letterSpacing: '0.14em',
            color: i % 7 === 0 ? '#f59e0b' : i % 7 === 1 ? '#34d399' : i % 7 === 2 ? '#60a5fa' : i % 7 === 3 ? '#c084fc' : i % 7 === 4 ? '#fb923c' : i % 7 === 5 ? '#f472b6' : '#14b8a6',
            opacity: 0.7,
          }}>
            {label}
          </span>
...
function TermCard({ term, onRelatedClick, highlight }: {
  ...
  return (
    ...
    >
      {/* Diagram section */}
      <div className="diagram-frame" style={{
        background: `linear-gradient(160deg, ${color}0e 0%, ${color}03 50%, rgba(4,4,8,1) 80%)`,
        borderBottom: `1px solid ${color}14`, position: 'relative',
      }}>
        <div style={{
          position: 'absolute', top: 8, right: 8, zIndex: 3,
          fontSize: '7px', fontWeight: 900, letterSpacing: '0.14em', padding: '3px 7px',
          borderRadius: 6, background: `${color}14`, color, border: `1px solid ${color}28`,
          textTransform: 'uppercase',
        }}>{CAT_SHORT[term.category]}</div>
        <div style={{ position: 'relative', zIndex: 1 }}>
          {DiagramComp
            ? <DiagramComp />
            : <div style={{ height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 28, opacity: 0.25 }}>{CAT_ICONS[term.category]}</span>
              </div>
          }
        </div>
      ...
      <div style={{ padding: '13px 14px 13px 12px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 6 }}>
          <h3 style={{ fontSize: 14, fontWeight: 900, color: 'rgba(255,255,255,0.96)', lineHeight: 1.2, flex: 1, letterSpacing: '-0.2px' }}>
            {term.term}
          </h3>
          {term.abbr && (
            <span style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.14em', padding: '3px 8px', borderRadius: 7, whiteSpace: 'nowrap', flexShrink: 0, background: `${color}16`, color, border: `1px solid ${color}30`, boxShadow: `0 0 8px ${color}12` }}>
              {term.abbr}
            </span>
          )}
        </div>
...
// ── Category showcase cards (bottom) ─────────────────────────────────────────
    ...
    <section style={{ padding: '0 20px 64px' }}>
      <div style={{ maxWidth: 1020, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <p style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.24em', textTransform: 'uppercase', color: 'rgba(100,116,139,0.35)', marginBottom: 8 }}>Explore by Category</p>
          <h2 style={{ fontSize: 'clamp(22px,4vw,34px)', fontWeight: 900, letterSpacing: '-1px', color: 'white' }}>
            7 Core ICT Frameworks
          </h2>
          <p style={{ fontSize: 13, color: 'rgba(100,116,139,0.7)', marginTop: 8, maxWidth: 420, margin: '8px auto 0' }}>
            Every concept organized by its role in the ICT methodology.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 12 }}>
          {CATEGORIES.map(cat => {
            ...
            // pick 3 representative terms
            const sample = TERMS.filter(t => t.category === cat).slice(0, 3).map(t => t.abbr || t.term.split(' ')[0])
            return (
              <button key={cat}
                onClick={() => { onSelect(isActive ? null : cat); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
                style={{
                  '--accent': color,
                  borderRadius: 18, padding: '22px 20px', textAlign: 'left', cursor: 'pointer',
                  background: isActive ? `${color}14` : 'rgba(5,5,14,0.97)',
                  border: `1px solid ${isActive ? color + '45' : color + '18'}`,
                  boxShadow: isActive ? `0 0 32px ${color}18` : 'none',
                ...
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = ''; (e.currentTarget as HTMLElement).style.boxShadow = isActive ? `0 0 32px ${color}18` : 'none' }}
              >
                {/* Glow top */}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, background: `${color}15`, border: `1px solid ${color}28`, flexShrink: 0 }}>
                    {CAT_ICONS[cat]}
                  </div>
                  <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                  {sample.map(s => (
                    <span key={s} style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.1em', padding: '3px 8px', borderRadius: 6, background: `${color}10`, color, border: `1px solid ${color}20` }}>
                      {s}
                    </span>
                  ))}
                  <span style={{ fontSize: 9, color: `${color}60`, padding: '3px 0', display: 'flex', alignItems: 'center' }}>+{count - 3} more →</span>
                </div>
              </button>
            )
          })}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#04040a', color: 'white', position: 'relative' }}>
      ...
      {/* ══ Navigation ══════════════════════════════════════════════════════════ */}
      <nav style={{
        borderBottom: '1px solid rgba(255,255,255,0.055)', position: 'sticky', top: 0, zIndex: 50,
        background: navShadow ? 'rgba(4,4,10,0.97)' : 'rgba(4,4,10,0.85)',
        backdropFilter: 'blur(24px)',
        boxShadow: navShadow ? '0 1px 40px rgba(0,0,0,0.7)' : 'none',
        ...
        <div style={{ maxWidth: 1020, margin: '0 auto', padding: '0 20px', height: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 10, color: 'rgba(100,116,139,0.6)', fontWeight: 600 }}>{TERMS.length} terms</span>
            ...
            <div style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.1em', padding: '4px 9px', borderRadius: 7, background: 'rgba(52,211,153,0.1)', color: '#34d399', border: '1px solid rgba(52,211,153,0.22)' }}>
              {TERMS.filter(t => t.diagramId).length} diagrams
            </div>
          </div>
        </div>
      </nav>
      ...
      {/* ══ Hero ════════════════════════════════════════════════════════════════ */}
      <section style={{ position: 'relative', padding: '80px 20px 56px', textAlign: 'center', overflow: 'hidden', zIndex: 1 }}>
        {/* Central glow */}
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse 80% 70% at 50% -5%, rgba(245,158,11,0.1) 0%, transparent 65%)' }}/>
        {/* Scrolling grid */}
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: 'linear-gradient(90deg,transparent,rgba(245,158,11,0.7),transparent)', pointerEvents: 'none' }}/>

        {/* Side decorations */}
        <div style={{ position: 'absolute', left: '2%', top: '15%', bottom: '15%', width: '140px', pointerEvents: 'none', display: 'flex', flexDirection: 'column', justifyContent: 'space-evenly', opacity: 0.6 }}>
          {CATEGORIES.slice(0, 4).map(cat => (
            <div key={cat} style={{ display: 'flex', alignItems: 'center', gap: 6, writingMode: 'horizontal-tb' }}>
              <div style={{ width: 20, height: 1, background: CATEGORY_COLORS[cat], opacity: 0.5 }}/>
              <span style={{ fontSize: 8, fontWeight: 900, letterSpacing: '0.15em', color: CATEGORY_COLORS[cat], textTransform: 'uppercase', opacity: 0.6, whiteSpace: 'nowrap' }}>{cat.split(' ')[0]}</span>
            </div>
          ))}
        </div>
        <div style={{ position: 'absolute', right: '2%', top: '15%', bottom: '15%', width: '140px', pointerEvents: 'none', display: 'flex', flexDirection: 'column', justifyContent: 'space-evenly', alignItems: 'flex-end', opacity: 0.6 }}>
          ...
          <div style={{ position: 'relative', maxWidth: 540, margin: '0 auto 28px' }}>
            ...
            {query && (
              <button onClick={() => setQuery('')}
                style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgba(148,163,184,0.5)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 20, lineHeight: 1 }}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 24, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={s.label} style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                ...
                <span style={{ fontSize: 9.5, color: 'rgba(100,116,139,0.6)', fontWeight: 600, letterSpacing: '0.07em' }}>{s.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {!query && (
        <section style={{ padding: '12px 20px 6px', position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: 1020, margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: 4, justifyContent: 'center', alignItems: 'center' }}>
            <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.15em', color: 'rgba(100,116,139,0.35)', textTransform: 'uppercase', marginRight: 4 }}>Jump</span>
        ...
        <div style={{ maxWidth: 1020, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {hasFilter && (
            <button onClick={clearFilters}
              style={{ fontSize: 10, fontWeight: 700, color: 'rgba(100,116,139,0.6)', background: 'none', border: '1px solid rgba(255,255,255,0.06)', cursor: 'pointer', padding: '4px 10px', borderRadius: 8, transition: 'all 0.15s' }}
              onMouseEnter={e => (e.currentTarget.style.color = 'rgba(255,255,255,0.7)')}
      ...
      {!hasFilter && (
        <>
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', marginBottom: 0 }}/>
        ...
        <div style={{ maxWidth: 1020, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: 10 }}>
            {[
              ...
              { val: `${TERMS.filter(t=>t.related.length>3).length}`,label: 'Linked',     color: '#14b8a6', icon: '🔗' },
            ].map(s => (
      ...
      {/* ══ ICT Quick reference strip ════════════════════════════════════════════ */}
      <section style={{ borderTop: '1px solid rgba(255,255,255,0.04)', padding: '40px 20px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 1020, margin: '0 auto' }}>
          <p style={{ textAlign: 'center', fontSize: 9, fontWeight: 900, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(100,116,139,0.25)', marginBottom: 16 }}>ICT Trading Flow</p>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap', gap: 0 }}>
            {[
              ...
              { label: 'OTE/FVG',  c: '#f59e0b' }, { label: '→', c: 'rgba(255,255,255,0.15)' },
          ...
          <p style={{ textAlign: 'center', fontSize: 11, color: 'rgba(100,116,139,0.4)', marginTop: 14, lineHeight: 1.7 }}>
            The ICT methodology flows from macro bias down to precision entries. Search any step above to learn more.
          </p>
        </div>
      </section>
